...
merge_cutup_clustering.py 04_Binning/concoct/your_sample/KR46_Sept_clustering_gt1000.csv > 04_Binning/concoct/your_sample/your_sample_clustering_merged.csv
...
Congratulations, you have bins! Repeat the above process for each of your samples.

Before we move onto binning with MetaBAT2, let's add the sample name and binning method to our bin fasta files.
...
for i in *.fa; do mv $i KR46_Sept_concoct_bin.$i; done
```
## MetaBAT2
3. Running MetaBAT2
```
...
Congraulations, you have more bins! Now we can move on to Bin QC.
...
The file "checkm2/quality_report.tsv" has our results. We can select the bins that pass our minimum completeness (50%) and maximum contamination (10%) levels with an `awk` command.
...
The file "checkm/your_sample_checkm.tsv" has our results. We can select the bins that pass our minimum completeness (50%) and maximum contamination (10%) levels with the `awk` command adjusted for the appropriate columns.


awk -F "\t" '$12 >=50 && $13 <=10' your_sample_checkm.tsv > your_sample_checkm_filtered.tsv

**Q: What is the benefit to using CheckM2 over CheckM?**
...
The input files are a list of contig headers and their associated bin.
...
```
The file "DASTool_summary.tsv" summarises the results. If you have any bins ending in "sub" these bins have been refined by DAS Tool. You can run CheckM2 again to see if the "sub" bins have improved.