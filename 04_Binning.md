...
Submit the job with an sbatch script.
...
We'll next do binning with MetaBAT2.
...
```
Again, you'll need to submit this with an sbatch script calling the metabat image and your metabat script. You can use most of the same SBATCH parameters as you used for concoct. The exception is that metabat is much quicker, so you can reduce to time to 2 hours (and it will still likely be quicker than that).

Congratulations, you now have more bins! You can find them in the **`04_binning/metabat/${sample}/bins_fasta`** directory. We added the sample name and binning method to our file names within the `metabat2` command so we don't need to rename our bin files like we are about to with our CONCOCT bins.

## Tidying up before we move on to BinQC
Before we move onto binQC, let's add the sample name and binning method to the names of our concoct bin files that are currently just 1.fa, 2.fa etc.

Move into the directory of the bin.fasta files you want to rename:

**`cd 04_Binning/concoct/${sample}/bins_fasta`**
...
```
Perform the above renaming step on the bins for each of your samples.
...
~/checkm2/bin/checkm2 predict --threads 6 --input 04_Binning/concoct/${sample}/bins_fasta/ -x fa --output-directory 04_Binning/concoct/${sample}/checkm2 \
--database_path CheckM2_database/uniref100.KO.1.dmnd

#CheckM2 on MetaBAT2 bins

~/checkm2/bin/checkm2 predict --threads 6 --input 04_Binning/metabat/${sample}/bins_fasta/ -x fa --output-directory 04_Binning/metabat/${sample}/checkm2 \
--database_path CheckM2_database/uniref100.KO.1.dmnd
...
The file **`04_Binning/concoct/${sample}/checkm2/quality_report.tsv`** contains our results. You should have one for each sample and each binning method. We can select the bins that pass our minimum completeness (50%) and maximum contamination (10%) thresholds with an `awk` command.
...
awk -F "\t" '$2 >=50 && $3 <=10' 04_Binning/concoct/${sample}/checkm2/quality_report.tsv > 04_Binning/concoct/${sample}/checkm2/${sample}_quality_report_filtered.tsv
...
Now do the same for your metabat bins by changing the path in the above script from concoct to metabat.

The file **`quality_report_filtered.tsv`** now contains a list of our bins that pass quality filtering. There should be two for each of your samples (one for metabat bins, one for concoct bins).

Let's download these files to our laptops. In the directory on your laptop that you would like to download the files to, make a folder called metabat and a folder called concoct

```
mkdir checkm2_metabat checkm2_concoct
```

```
scp 'username@jed.epfl.ch:/scratch/username/your_dataset/04_binning/concoct/*/checkm2/*_quality_report_filtered.tsv' concoct/
```

```
scp 'username@jed.epfl.ch:/scratch/username/your_dataset/04_binning/metabat/*/checkm2/*_quality_report_filtered.tsv' metabat/
```

We did this as the file names are the same, so if you had copied them to your current directory (with `.`) they second set you downloaded would have overwritten the first.
...
checkm lineage_wf -f 04_Binning/concoct/${sample}/checkm/${sample}_checkm.tsv \
04_Binning/concoct/${sample}/bins_fasta --tab_table -t 20 -x fa 04_Binning/concoct/${sample}/checkm/

#CheckM on MetaBAT2 bins

checkm lineage_wf -f 04_Binning/metabat/${sample}/checkm/${sample}_checkm.tsv \
04_Binning/metabat/${sample}/bins_fasta --tab_table -t 20 -x fa 04_Binning/metabat/${sample}/checkm/
```

CheckM takes a bit longer to run than CheckM2:
...
The file **`checkm/your_sample_checkm.tsv`** contains our results. We can select the bins that pass our minimum completeness (50%) and maximum contamination (10%) levels with the same `awk` command adjusted for the appropriate columns (the columns of the output file from CheckM are in a different order to CheckM2).

```
awk -F "\t" '$12 >=50 && $13 <=10' 04_Binning/metabat/your_sample/checkm/${sample}_checkm.tsv > 04_Binning/metabat/your_sample/checkm/your_sample_checkm_filtered.tsv
```

Repeat the above filtering for your CheckM output on concoct bins. The file **`your_sample_checkm_filtered.tsv`** now contains a list of our bins that pass quality filtering.
...
```
DAS_Tool -i 04_Binning/dastool/your_sample_concoct.contigs2bin.tsv,04_Binning/dastool/your_sample_metabat.contigs2bin.tsv -l concoct,metabat -c 02_Assembly/filtered_contigs/your_sample_contigs.fa -o 04_Binning/dastool/your_sample/your_sample --write_bins --threads 6
```
